...
#define blk0(i) (block->l[i] = (ROLL(block->l[i],24)&0xFF00FF00)|(ROLL(block->l[i],8)&0x00FF00FF))

#define blk(i) (block->l[i&15] = ROLL(block->l[(i+13)&15]^block->l[(i+8)&15]^block->l[(i+2)&15]^block->l[i&15],1))

/* (R0+R1), R2, R3, R4 are the different operations used in SHA1 */
#define R0(v,w,x,y,z,i) z+=((w&(x^y))^y)+blk0(i)+0x5A827999+ROLL(v,5);w=ROLL(w,30);
#define R1(v,w,x,y,z,i) z+=((w&(x^y))^y)+blk(i)+0x5A827999+ROLL(v,5);w=ROLL(w,30);
#define R2(v,w,x,y,z,i) z+=(w^x^y)+blk(i)+0x6ED9EBA1+ROLL(v,5);w=ROLL(w,30);
#define R3(v,w,x,y,z,i) z+=(((w|x)&y)|(w&x))+blk(i)+0x8F1BBCDC+ROLL(v,5);w=ROLL(w,30);
#define R4(v,w,x,y,z,i) z+=(w^x^y)+blk(i)+0xCA62C1D6+ROLL(v,5);w=ROLL(w,30);
